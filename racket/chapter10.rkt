...
(defun defun.align ()
  (J-Bob/define
   ...
   '(((dethm natp/wt (x)
      ...
      ((E A A A) (if-true (equal (natp (+ (+ (wt (car x)) (wt (car x)))
                                          (wt (cdr x))))
                                 't)
                          't))
      ((E A A A Q) (natp/+ (wt (car x)) (wt (car x))))
      ((E A A A A) (if-true (equal (natp (+ (+ (wt (car x)) (wt (car x)))
                                            (wt (cdr x))))
                                   't)
                            't))
      ((E A A A A Q) (equal-if (natp (wt (cdr x))) 't))
      ((E A A A A A 1) (natp/+ (+ (wt (car x)) (wt (car x)))
                               (wt (cdr x))))
      ((E A A A A A) (equal-same 't))
     ...
     ((dethm positive/wt (x)
      ...
      ((E A A 1 2) (if-nest-E (atom x)
                              ...
                              (+ (+ (wt (car x)) (wt (car x)))
                                 (wt (cdr x)))))
      ((E A A) (if-true (equal (< '0 (+ (+ (wt (car x))
                                           (wt (car x)))
                                        (wt (cdr x))))
                               't)
                        't))
      ((E A A Q) (equal-if (< '0 (wt (car x))) 't))
      ((E A A A) (if-true (equal (< '0 (+ (+ (wt (car x))
                                           (wt (car x)))
                                        (wt (cdr x))))
                               't)
                          't))
      ((E A A A Q) (equal-if (< '0 (wt (cdr x))) 't))
      ((E A A A A) (if-true (equal (< '0 (+ (+ (wt (car x))
                                           (wt (car x)))
                                        (wt (cdr x))))
                               't)
                          't))
      ((E A A A A Q) (positives-+ (wt (car x)) (wt (car x))))
      ((E A A A A A 1) (positives-+ (+ (wt (car x))
                                       (wt (car x)))
                                    (wt (cdr x))))
      ((E A A A A A) (equal-same 't))
      ((E A A A A) (if-same (< '0 (+ (wt (car x)) (wt (car x)))) 't))
      ((E A A A) (if-same (< '0 (wt (cdr x))) 't))
     ...
     ((defun align (x)
      ...
      ((E E 1 Q) (atom/cons (car (car x))
                            (cons (cdr (car x)) (cdr x))))
      ((E E 1) (if-false '1
                         (+ (+ (wt (car (cons (car (car x))
                                              (cons (cdr (car x))
                                                    (cdr x)))))
                               (wt (car (cons (car (car x))
                                              (cons (cdr (car x))
                                                    (cdr x))))))
                            (wt (cdr (cons (car (car x))
                                           (cons (cdr (car x))
                                                 (cdr x))))))))
      ((E E 1 1 1 1) (car/cons (car (car x))
                               (cons (cdr (car x)) (cdr x))))
      ((E E 1 1 2 1) (car/cons (car (car x))
                               (cons (cdr (car x)) (cdr x))))
      ((E E 1 2 1) (cdr/cons (car (car x))
                             (cons (cdr (car x)) (cdr x))))
      ((E E 1 2) (wt (cons (cdr (car x)) (cdr x))))
      ((E E 1 2 Q) (atom/cons (cdr (car x)) (cdr x)))
      ((E E 1 2) (if-false '1
                           (+
                            (+ (wt (car (cons (cdr (car x))
                                              (cdr x))))
                               (wt (car (cons (cdr (car x))
                                              (cdr x)))))
                            (wt (cdr (cons (cdr (car x))
                                           (cdr x)))))))
      ((E E 1 2 1 1 1) (car/cons (cdr (car x))
                                 (cdr x)))
      ((E E 1 2 1 2 1) (car/cons (cdr (car x))
                                 (cdr x)))
      ((E E 1 2 2 1) (cdr/cons (cdr (car x))
                               (cdr x)))
      ((E E 2) (wt x))
      ...
      ((E E 1) (associate-+ (+ (wt (car (car x))) (wt (car (car x))))
                            ...
                            (wt (cdr x))))
      ((E E) (common-addends-< (+ (+ (wt (car (car x)))
                                     (wt (car (car x))))
                                  (+ (wt (cdr (car x)))
                                     (wt (cdr (car x)))))
                               (+ (+ (+ (wt (car (car x)))
      ...
      ((E E 1) (associate-+ (+ (wt (car (car x))) (wt (car (car x))))
                            (wt (cdr (car x)))
                            (wt (cdr (car x)))))
      ((E E 1 1) (commute-+ (+ (wt (car (car x))) (wt (car (car x))))
                            (wt (cdr (car x)))))
      ((E E 1) (associate-+ (wt (cdr (car x)))
                            (+ (wt (car (car x)))
                               (wt (car (car x))))
                            (wt (cdr (car x)))))
...
(defun dethm.align/align ()
  (J-Bob/define
   ...
   '(((dethm align/align (x)
      ...
      ((E A) (if-same (equal (align (align (cdr x)))
                             (align (cdr x)))
                      't))
      ((E E A 1 1) (align x))
      ...
      ((E E A 1 1) (if-nest-E (atom (car x))
                              (cons (car x)
                                    (align (cdr x)))
                              (align (rotate x))))
      ((E E A 1) (equal-if (align (align (rotate x)))
                           (align (rotate x))))
      ((E E A 2) (align x))
      ...
      ((E E) (if-same (equal (align (align (rotate x)))
                             (align (rotate x)))
                      't))
      ((E) (if-same (atom (car x)) 't))
      (() (if-same (atom x) 't))))))